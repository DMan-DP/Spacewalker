	...
	public class XRHandControllerLink : HandControllerLink
	{
		...
		protected override void Start()
		{
			...
			if (grabButton == squeezeButton)
				Debug.LogError(
					"AUTOHAND: You are using the same button for grab and squeeze on HAND CONTROLLER LINK, this may create conflict or errors",
					this);
		...
		private void Update()
		{
			...
			if (device != null && device.isValid)
			{
				...
				//Grab input
				if (device.TryGetFeatureValue(GetCommonButton(grabButton), out var grip))
				{
					if (grabbing && !grip)
					{
						hand.Release();
				...
				//Squeeze input
				if (device.TryGetFeatureValue(GetCommonButton(squeezeButton), out var squeeze))
				{
		...
		public bool ButtonPressed(CommonButton button)
		{
			...
			if (device.TryGetFeatureValue(GetCommonButton(button), out var pressed)) return pressed;
		...
		public float GetAxis(CommonAxis axis)
		{
			...
			if (device.TryGetFeatureValue(GetCommonAxis(axis), out var axisValue)) return axisValue;
			return 0;
		}
		...
		public Vector2 GetAxis2D(Common2DAxis axis)
		{
			...
			if (device.TryGetFeatureValue(GetCommon2DAxis(axis), out var axisValue)) return axisValue;
			return Vector2.zero;
		}
